...
f <- "data/epsa2021-program.tsv"
if (!fs::file_exists(f)) {

  "https://raw.githubusercontent.com/briatte/epsa2021/master/data/program.tsv" %>%
...
# - e.g. Stiller in session 101, abstracts 0336 and 0382
# - e.g. Kjøller Larsen in session 23, abstracts 0050 and 0067
#
# happens only to presenters, not chairs or discussants
stopifnot(e$role[ duplicated(e) ] == "p")
#
# happens only very few times (n = 13 duplicates)
e[ duplicated(e), ]
#
# relative frequency: 2.5% of all authors (n = 1001)
2 * sum(duplicated(e)) / sum(e$role == "p")

# export
readr::write_tsv(e, "data/epsa2021-edges.tsv")
...
select(d, i = pid, full_name, affiliation) %>%
  ...
  distinct() %>%
  # export
  readr::write_tsv("data/epsa2021-participants.tsv")
...
select(d, j = session_id, starts_with("session_")) %>%
  ...
  distinct() %>%
  # export
  readr::write_tsv("data/epsa2021-panels.tsv")
...
select(d, starts_with("abstract_")) %>%
  ...
  distinct() %>%
  # export
  readr::write_tsv("data/epsa2021-abstracts.tsv")
